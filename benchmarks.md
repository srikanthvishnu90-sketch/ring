...
- **Thread ownership + export/delete repaired** (commit `ff8e369`, deployed): all `/api/threads` routes now require auth + membership (`members` jsonb contains user id; creator auto-added; group threads shared via members); webhook threads carry sender names, never collide with user ids; fixed `sbRequest` argument-order bug that silently broke `/api/export`, `/api/account`, and four `/api/devices` reads; export now includes threads/messages via membership; account delete removes sole-member threads + messages, removes the user from shared threads, wipes all app rows, and deletes the Supabase Auth user via the admin API. Verified: two-user isolation (A sees A+AB, B sees only AB, nobody sees nothing), unauth thread/export routes 401 in prod, PostgREST contains-filter live.
- **Voice retention + speed** (commit `d312cdf`, deployed): raw audio is never persisted — memory only for the request (policy: `docs/voice-retention.md`, surfaced in the app Privacy sheet); transcripts persist only with `threadId` or `saveTranscript:true`, otherwise ephemeral; stored transcripts follow export/delete. Speed path: ring-native pcm16 (16 kHz mono) accepted and WAV-wrapped server-side (byte-exact, verified); voice agent turns use speak-aloud style + 150-token cap; per-stage timings returned. New `POST /api/voice/stream` (SSE: transcript/token/audio/done events) for incremental render/playback. Voice agent acts in the turn: low-risk tools execute, medium/high held as tap-to-approve cards, spoken reply names what's held. Verified: 400/501 + SSE error paths live in prod. **Needs Vishnu:** STT/TTS credentials (`OPENAI_BASE_URL` + `OPENAI_API_KEY` with audio endpoints, or compatible provider) before real voice turns work.
